...
export default function MovableLineMeasureArea({
  ...
  React.useEffect(() => {
    if (moveableRef.current) {
      ...
      if (
        measureValues &&
        measureValues.offsetHeight &&
        measureValues.offsetWidth
      ) {
        ...
        moveableRef.current.moveable.request(
          "resizable",
          {
            offsetWidth: measureValues.offsetWidth,
            offsetHeight: measureValues.offsetHeight,
          },
          true
        );
      }
      ...
      if (measureValues && measureValues.x && measureValues.y) {
        moveableRef.current.moveable.request(
          "draggable",
          { x: measureValues.x, y: measureValues.y },
          true
        );
      }
  ...
  function onFluidEdge(e) {
    ...
    const value = posToValue(
      ...
      measureValues.minValue,
      measureValues.maxValue
    );
    setValue(value);
  }
  return (
      ...
      <Moveable
        ...
        onRotateEnd={(e) => {
          if (!measureValues.rotation) {
            ...
            Math.abs(measureValues.rotation - e.lastEvent.rotate) > 0.001
          ) {
            setMeasureValues({
              ...measureValues,
              rotation: e.lastEvent.rotate,
